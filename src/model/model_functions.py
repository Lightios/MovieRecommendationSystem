...
def get_predictions(user_id, last_movie_ratings, movies, len_genres, len_tags, user_movie_matrix, user_movie_matrix_normalized, user_means):
...
def get_average_movie(last_movie_ratings, movies, len_genres, len_tags):
    ...
    last_movie_ratings['tagId'] = last_movie_ratings['movieId'].apply(lambda x: movies.loc[movies['movieId'] == x, 'tagId'].values[0])
    ...
    if last_movie_ratings['rating'].max() <= 2.5:
        ...
        for id in last_movie_ratings['movieId']:
            ...
            average_tags = average_tags + np.array(movie_tags) * last_movie_ratings.loc[last_movie_ratings['movieId'] == id, 'rating'].values[0] / ratings_sum
        return average_tags
